...
void input() {
	...
	partial_log_sum.resize(n + 1); // partial_log_sum[i]: index가 i 이전 확률까지의 합
	partial_log_sum[0] = 0;
	for (int i = 0; i < n; i++){
		if (sorted_p[i] != 1)
			partial_log_sum[i + 1] = partial_log_sum[i] + logl(1 - sorted_p[i]);
		else
			partial_log_sum[i + 1] = partial_log_sum[i];
	}
	one_count.clear();
...
long double solve() {
	...
	for(int i = 0; i < n; i++) { //a_i is fixed
		int tour = 0;
		while(CCW(pivot, i) <= 0 && tour != n){
			//cout << "while " << pivot << " " << i << " " << CCW(pivot, i) << endl;
